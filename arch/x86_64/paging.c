...
void paging_map(void *map, uint64_t phys, uint64_t virt, uint64_t size) {
    while (size > 0) {
        ...
        uint64_t l4_index = (virt >> 27) & ((1<<9)-1);
        uint64_t l3_index = (virt >> 18) & ((1<<9)-1);
        uint64_t l2_index = (virt >> 9) & ((1<<9)-1);
	uint64_t l1_index = virt & ((1<<9)-1);

        uint64_t *l4_entry = (uint64_t *) map;
        if (l4_entry[l4_index] == 0) {
            l4_entry[l4_index] = ((uint64_t) malloc_pages(1)) | 0b11;
	    memset((void *) (l4_entry[l4_index] & 0x0007fffffffff000), 0, PAGE_SIZE);
        }

        uint64_t *l3_entry = (uint64_t *) l4_entry[l4_index];
        if (l3_entry[l3_index] == 0) {
            l3_entry[l3_index] = ((uint64_t) malloc_pages(1)) | 0b11;
            memset((void *) (l3_entry[l3_index] & 0x0007fffffffff000), 0, PAGE_SIZE);
        }

        uint64_t *l2_entry = (uint64_t *) l3_entry[l3_index];
        if (l2_entry[l2_index] == 0) {
            l2_entry[l2_index] = ((uint64_t) malloc_pages(1)) | 0b11;
            memset((void *) (l2_entry[l2_index] & 0x0007fffffffff000), 0, PAGE_SIZE);
        }

	uint64_t *l1_entry = (uint64_t *) l3_entry[l3_index];
        l1_entry[l1_index] = (phys & 0x0007fffffffff000) | 0b11;
